...
public class SortByMake {
   ...
   */
  public static void mergeSortMake(Car[] data, int low, int high) {
    ...
    if (low < high) {
      ...
      // Recursive mergeSort
      mergeSortMake(data, low, mid); // Sort the left side
      mergeSortMake(data, mid + 1, high); // Sort the right side
  ...
  private static void merge(Car[] data, int start, int mid, int end) {
    ...
    while (pos1 <= mid && pos2 <= end) {
      if (data[pos2].getMake().compareTo(data[pos1].getMake()) < 0) { // Element in right list (pos2) is smaller
